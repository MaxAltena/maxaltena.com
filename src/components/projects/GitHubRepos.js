...
export default class GitHubRepos extends Component {
	...
	getReposFromOrgs = () => {
		...
		axios.get("https://api.github.com/users/" + username + "/orgs").then(orgs => {
			orgs.data.forEach(org => {
				this.setState({
					organisations: [...this.state.organisations, org]
				});
				localStorage.setItem("gh-orgs", JSON.stringify(this.state.organisations));
				axios
					...
					.then(repos => {
						repos.data.forEach(repo => {
							if (!repo.archived)
								axios
									.get(
										"https://api.github.com/repos/" + org.login + "/" + repo.name + "/contributors"
									)
									.then(contris => {
										if (contris.data !== "")
			...
			.then(repos => {
				repos.data.forEach(repo => {
					if (!repo.archived) {
						...
						localStorage.setItem("gh-repos", JSON.stringify(this.state.repositories));
					}
				});
			});
	};
	...
	render() {
		...
		return (
			<>
				...
				{projects.length >= 1 ? (
					<div className="gh-projectsholders">{this.renderProjectsGitHub(projects)}</div>
				) : (
					<>
						<Loading />
